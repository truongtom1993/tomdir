...
function getGlobalGitignorePath() {
	try {
		...
		// Git khÃ´ng cÃ i Ä‘áº·t hoáº·c khÃ´ng cÃ³ config
	}
	
	// Fallback: thá»­ cÃ¡c Ä‘Æ°á»ng dáº«n máº·c Ä‘á»‹nh phá»• biáº¿n
	const defaultPaths = [
...
export function listDirectory(dir, ignoreGitignore = false) {
	...
	rl.question('Sao chÃ©p vÃ o clipboard? (y/n): ', answer => {
		if (answer.toLowerCase() === 'y') {
			try {
				clipboard.writeSync(result);
				console.log('ÄÃ£ sao chÃ©p vÃ o clipboard.');
			} catch (err) {
				console.error('Sao chÃ©p tháº¥t báº¡i: ', err);
			}
		}
		rl.close();
	});
