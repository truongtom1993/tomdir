...
function getGlobalGitignorePath() {
	try {
		...
		if (configPath) {
			...
			const expandedPath = configPath.startsWith('~') ? path.join(os.homedir(), configPath.slice(1)) : configPath;
			return expandedPath;
		}
	} catch (err) {
		console.info(`🟠 lib/index.js	Line:45	ID:0d17a9`, err);
	}
...
export function listDirectory(dir, ignoreGitignore = false) {
	...
	process.stdout.write('Sao chép vào clipboard? (y/n): ');

	process.stdin.once('data', key => {
		const answer = key.toLowerCase();

		// In ký tự người dùng nhập
